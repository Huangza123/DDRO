    ...
    for epoch in range(EPOCH):
        if epoch<100:
            ...
            if epoch==100:
                ...
                seed_neighbor=torch.zeros((3,3,minority_data.size(2),minority_data.size(3))) #3*28*28
                ...
                for label_idx in glabel_idx:
                    ...
                    for g in tqdm(range(int(num_max-weights[label_idx])),ncols=80):
                        alpha=torch.rand((seed_neighbor.size(0),3,32,32),requires_grad=True) #3*1
                        ...
                        dis=torch.sum((seed.reshape(-1,3*32*32)-_data.reshape(-1,3*32*32))**2,axis=1) #674
                        ...
                        for gepoch in range(10):
                            ...
                            plt.pause(0.01)
                            plt.clf()"""
                            goutput=net(generated_sample.view(-1,3,generated_sample.size(1),generated_sample.size(2)))
                            
                            glabel=torch.tensor([label_idx]).long().to(device)
                            gloss=torch.exp(-gloss_func(goutput,glabel))
                            
                            goutput_seed=net(seed.view(-1,3,32,32).to(device))
                        ...
                        train_data_gen.append(proposal_generate_sample.detach().cpu().numpy().reshape(-1,32,32))
