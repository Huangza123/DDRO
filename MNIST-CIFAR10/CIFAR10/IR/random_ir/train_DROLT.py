...
class DROLoss(nn.Module):
    ...
    def forward(self, batch_feats, batch_targets, centroid_feats, centroid_targets):
        ...
        if self.class_weights is not None:
            weights = self.class_weights[centroid_classes].to(device)
...
def centroid(net,trainloader,device):
    
    features=torch.empty((0,net.fc.in_features)).to(device)
    targets=torch.empty(0,dtype=torch.long).to(device)
    with torch.no_grad():
        avgpool_out=[]
        for i,(inputs,target) in enumerate(trainloader):
            for name,module in net.named_children():
                feats=module(inputs.to(device))
                if 'avgpool' in name:
                    #print(feats.data.shape,'feat')
                    feats=feats.data.reshape(inputs.shape[0],net.fc.in_features)
                    break
                inputs=feats
            #outputs=net(inputs)
            # print(avgpool_out[0].shape)
            features=torch.cat((features,feats))
            targets=torch.cat((targets,target.to(device).long()))
...
def train_drolt(net,trainloader,testloader,num_classes,path):
    ...
    feat_optim = torch.optim.Adam(feat_params + [learnable_epsilons], lr=0.001,betas=(0.9, 0.999), eps=1e-08, weight_decay=0.001, amsgrad=False)
    cls_optim = torch.optim.Adam(cls_params, lr=0.001,betas=(0.9, 0.999), eps=1e-08, weight_decay=0.001, amsgrad=False)
    ...
    """for i in range(len(weight)):
        if len(data_all[i])!=max_weight:
            ...
            for k in rand_idx:
                #print(data_all[i][k].shape,label_all[i][k].shape)
    ...
    for epoch in range(EPOCH):
        # if epoch%10==0:
        features,targets=centroid(net, trainloader,device)
        ...
        if epoch<feature_extractor_duration:    
            ...
            for i,(data,labels) in enumerate(trainloader):
                ...
                feats=torch.zeros(data.shape[0],512).to(device)
                for name,module in net.named_children():
                    feats=module(data.to(device))
                    if 'avgpool' in name:
                        feats=feats.data.reshape(data.shape[0],net.fc.in_features)
                        break    
                    data=feats
                ...
                for idx,loss_func in enumerate(train_feat_losses):
                    if type(loss_func)==nn.CrossEntropyLoss:
                        l_ce=loss_func(outputs,labels.long().to(device))
            ...
            for i,(data,labels) in enumerate(trainloader):
                ...
                for idx,loss_func in enumerate(train_class_losses):
                    if type(loss_func)==nn.CrossEntropyLoss:
                        l_ce=loss_func(outputs,labels.long())
                    elif type(loss_func)==DROLoss:
                        l_dro=loss_func(feats,labels,features,targets)
...
def train(path):
    ...
    testloader=Data.DataLoader(dataset=torch_test_dataset,
                                batch_size=200,
                                shuffle=True)
    train_drolt(net,trainloader,testloader,10,path)
