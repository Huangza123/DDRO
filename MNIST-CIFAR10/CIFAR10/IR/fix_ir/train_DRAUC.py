...
def attack_DRO(model, f, x, y, _lambda, attack_lr, \
    ...
    model.eval()
    max_loss = torch.zeros(y.shape[0]).cuda(1)
    max_delta = torch.zeros_like(x).cuda(1)
    #max_loss = torch.zeros(y.shape[0],1)
    #max_delta = torch.zeros_like(x)
    
    delta = torch.zeros_like(x).cuda(1) #init delta
...
def cv_train_au(net,trainloader,testloader,attr_count,num_classes,path):
    ...
    for i in range(1):
        ...
        best_metrics=train_drauc(net,trainloader,testloader,attr_count,num_classes,path)
...
def train(path):
    ...
    testloader=Data.DataLoader(dataset=torch_test_dataset,
                                batch_size=200,
                                shuffle=True)
    train_drauc(net,trainloader,testloader,10,path)
