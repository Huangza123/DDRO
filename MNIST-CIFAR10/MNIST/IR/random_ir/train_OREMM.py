...
def batch_oremm(inputs,labels):
    ...
    for lb in pos_label_list:
        ...
        #sampling_count=3
        #print(pos_data)
        pos_data=np.array(pos_data).reshape(-1,28*28)
        neg_data=np.array(neg_data).reshape(-1,28*28)
    ...
    oremm_data=torch.from_numpy(np.array(resampling_data)).float().reshape(-1,1,28,28)
...
def train_oremm(net,trainloader,testloader,num_classes,path):
    ...
    data_all=np.array(data_all).reshape(-1,28*28)
