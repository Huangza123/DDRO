    ...
    for epoch in range(EPOCH):
        for _,data in enumerate(trainloader):
            ...
            for i in np.unique(labels.cpu().numpy()):
                ...
                sigma_y=torch.zeros((512,512)).cuda(1)
                for c,mean in [(y_feature,mean_y)]:
                    s_c=torch.zeros((512,512)).cuda(1)
                    for sample in c:
                ...
                for z in np.unique(labels.cpu().numpy()):
                    ...
                    if z!=i:
                        w_z=net.fc.weight[z]
                        #print(net.fc.bias[z],'bias')
                        #unit_w_y_w_z=(w_y-w_z)/torch.sqrt(torch.sum((w_y-w_z)**2))
                        ds_yz=torch.mm(torch.mm((w_y-w_z).reshape(1,512),sigma_y),(w_y-w_z).reshape(512,1))/(torch.norm(w_y-w_z))**2
                        ds_y+=ds_yz
                        #print(ds_y)
                ...
                for p in probability[label_idx][:,i]:
                    if p>=tau:
                       ...
                       loss_i0=-torch.log(p)*weight_i0
                       #print(weight_i0,'>=tau')
                       loss+=loss_i0
                       count+=1
                    else:
                       g1=(1-probability[label_idx][:,i].mean())**r
                       weight_i1=1./(ds_y+1e-8)*(1.-beta)/(1.-beta**(n_y))*g1
                       loss_i1=-torch.log(p)*weight_i1
                       #print(weight_i1,'<tau')
                       loss+=loss_i1
                       count+=1
